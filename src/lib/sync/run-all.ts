...
export async function runFullSync(opts?: { trigger?: "cron" | "manual" }): Promise<SyncResult> {
  ...
  // ── 10b. Garde-fou coût chat IA : estime la dépense Mistral du mois, mail si ≥ limite ──
  // (cf. src/lib/chat/cost.ts — le plafond dur est dans la console Mistral, ceci ne fait qu'alerter)
  await step("chat_cost", "Coût chat IA (Mistral)", async () => {
    const { checkChatCost } = await import("@/lib/chat/cost")
