...
// Sections SANS connecteur hors FR → sur tout autre marché on n'affiche aucune donnée FR (état vide).
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const gated = market !== "FR" && !!section && FR_ONLY_SECTIONS.has(section)
  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-zinc-50 lg:ml-64">
        {gated ? (
          // Sur un marché gated, on ne rend pas la page (état vide) MAIS on garde le
          // Header commun → le switch marché reste accessible (sinon on est
          // coincé hors FR sans pouvoir revenir en FR).
          <>
