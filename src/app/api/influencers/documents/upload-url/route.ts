...
// Crée la ligne document + une URL d'upload signée (même patron que les factures).
export async function POST(request: Request) {
  try {
    const body = await request.json().catch(() => ({}))
    const fileName = (body.file_name || "").replace(/[^a-zA-Z0-9._-]/g, "_").slice(0, 150)
    const mime = (body.mime_type as string) || "application/octet-stream"
    ...
    await supabase.from("influencer_documents").update({ storage_path: path }).eq("id", row.id)
    ...
    return NextResponse.json({ document_id: row.id, bucket: BUCKET, path, upload_url: signed.signedUrl, token: signed.token })
  } catch (err) {
